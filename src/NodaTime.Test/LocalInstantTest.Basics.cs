...
namespace NodaTime.Test
{
    ...
    public partial class LocalInstantTest
    {
        ...
        {
            Assert.AreEqual("2002-06-09T00:00:00.000Z", new LocalInstant(TestTimeNow).ToString());
            Assert.AreEqual("2002-04-05T12:24:00.000Z", new LocalInstant(TestTime1).ToString());
            Assert.AreEqual("2003-05-06T14:28:00.000Z", new LocalInstant(TestTime2).ToString());
        }

        [Test]
        public void DateTimeFieldTypeIndexer()
        {
            var test = new LocalInstant(TestTime1); // 2002-06-09
        ...
        {
            var test = new LocalInstant(TestTimeNow); // 2002-06-09
        ...
        {
            var test1a = new LocalInstant(TestTime1);
            var test1b = new LocalInstant(TestTime1);
            ...
            var test2 = new LocalInstant(TestTime2);
            ...
            Assert.IsTrue(test1a.Equals(new MockLocalInstant()));
            Assert.IsFalse(test1a.Equals(new DateTime(TestTime1, IsoChronology.GetInstance(London))));
        }

        private class MockLocalInstant : AbstractLocalInstant
        {
        ...
        [Test]
        public void CompareTo_LocalInstant()
        {
            var test1a = new LocalInstant(TestTime1);
            var test1b = new LocalInstant(TestTime1);
        ...
        {
            var test1 = new LocalInstant(TestTime1);
            var test2a = new LocalInstant(TestTime2);
            ...
            Assert.AreEqual(+1, test2a.CompareTo(new MockLocalInstant()));
            Assert.AreEqual(0, test1.CompareTo(new MockLocalInstant()));
        }

        public void CompareTo_NullArgument_ThrowsException()
        {
            var test = new LocalInstant(TestTime1);
        ...
        {
            Assert.IsTrue(new LocalInstant(TestTime1).IsEqual(TestTime2));
            Assert.IsTrue(new LocalInstant(TestTime1).IsEqual(TestTime1));
            Assert.IsFalse(new LocalInstant(TestTime2).IsEqual(TestTime1));
        }
        ...
        public void IsEqualNow()
        {
            using (TestClock.ReplaceCurrent(TestTimeNow))
            {
                Assert.IsFalse(new LocalInstant(TestTimeNow - 1).IsEqualNow());
                Assert.IsTrue(new LocalInstant(TestTimeNow).IsEqualNow());
                Assert.IsFalse(new LocalInstant(TestTimeNow + 1).IsEqualNow());
            }
        }
        ...
        /// </summary>
        [Test]
        public void IsEqual_LocalInstant()
        {
            var test1 = new LocalInstant(TestTime1);
            var test1a = new LocalInstant(TestTime1);
            ...
            Assert.IsFalse(test2.IsEqual(new MockLocalInstant()));
            Assert.IsTrue(test1.IsEqual(new MockLocalInstant()));

            Assert.IsFalse(new LocalInstant(TestTimeNow + 1).IsEqual(null));
            Assert.IsTrue(new LocalInstant(TestTimeNow).IsEqual(null));
            Assert.IsFalse(new LocalInstant(TestTimeNow - 1).IsEqual(null));
        }
        ...
        {
            Assert.IsTrue(new LocalInstant(TestTime1).IsBefore(TestTime2));
            Assert.IsFalse(new LocalInstant(TestTime1).IsBefore(TestTime1));
            Assert.IsFalse(new LocalInstant(TestTime2).IsBefore(TestTime1));
        }
        ...
        public void IsBeforeNow()
        {
            using (TestClock.ReplaceCurrent(TestTimeNow))
            {
                Assert.IsTrue(new LocalInstant(TestTimeNow - 1).IsBeforeNow());
                Assert.IsFalse(new LocalInstant(TestTimeNow).IsBeforeNow());
                Assert.IsFalse(new LocalInstant(TestTimeNow + 1).IsBeforeNow());
            }
        }
        ...
        /// </summary>
        [Test]
        public void IsBefore_LocalInstant()
        {
            var test1a = new LocalInstant(TestTime1);
            var test1b = new LocalInstant(TestTime1);
            ...
            Assert.IsFalse(test2a.IsBefore(new MockLocalInstant()));
            Assert.IsFalse(test1a.IsBefore(new MockLocalInstant()));

            Assert.IsFalse(new LocalInstant(TestTimeNow + 1).IsBefore(null));
            Assert.IsFalse(new LocalInstant(TestTimeNow).IsBefore(null));
            Assert.IsTrue(new LocalInstant(TestTimeNow - 1).IsBefore(null));
        }

        [Test]
        public void IsAfter_long()
        {
            Assert.IsFalse(new LocalInstant(TestTime1).IsAfter(TestTime2));
            Assert.IsFalse(new LocalInstant(TestTime1).IsAfter(TestTime1));
            Assert.IsTrue(new LocalInstant(TestTime2).IsAfter(TestTime1));
        }
        ...
        public void IsAfterNow()
        {
            using (TestClock.ReplaceCurrent(TestTimeNow))
            {
                Assert.IsFalse(new LocalInstant(TestTimeNow - 1).IsAfterNow());
                Assert.IsFalse(new LocalInstant(TestTimeNow).IsAfterNow());
                Assert.IsTrue(new LocalInstant(TestTimeNow + 1).IsAfterNow());
            }
        }
        ...
        /// </summary>
        [Test]
        public void IsAfter_LocalInstant()
        {
            var test1a = new LocalInstant(TestTime1);
            var test1b = new LocalInstant(TestTime1);
            ...
            Assert.IsTrue(test2a.IsAfter(new MockLocalInstant()));
            Assert.IsFalse(test1a.IsAfter(new MockLocalInstant()));
        }

        [Test]
        public void IsAfter_NullLocalInstant_ComparesWithNow()
        {
            using (TestClock.ReplaceCurrent(TestTimeNow))
            {
                Assert.IsTrue(new LocalInstant(TestTimeNow + 1).IsAfter(null));
                Assert.IsFalse(new LocalInstant(TestTimeNow).IsAfter(null));
                Assert.IsFalse(new LocalInstant(TestTimeNow - 1).IsAfter(null));
            }
        }
        ...
        {
            var test = new LocalInstant(TestTimeNow);
            byte[] bytes;
            ...
            {
                var result = (LocalInstant) formatter.Deserialize(stream);
        ...
        [Test]
        public void ToLocalInstant_IsNoOp()
        {
            var test = new LocalInstant(TestTime1);
            var result = test.ToLocalInstant();
        ...
        //    LocalInstant test = new LocalInstant(TestTime1);
        ...
        public void ToDateTime_WithDateTimeZone()
        {
            ...
            test = new LocalInstant(TestTime1);
        ...
        public void WithDurationAdded_Int64AndInt()
        {
            ...
            var expected = new LocalInstant(TestTime1 + 123456789L);
            ...
            expected = new LocalInstant(TestTime1 + (2L * 123456789L));
            Assert.AreEqual(expected, result);

            result = test.WithDurationAdded(123456789L, -3);
            expected = new LocalInstant(TestTime1 - (3L * 123456789L));
            Assert.AreEqual(expected, result);
        }
