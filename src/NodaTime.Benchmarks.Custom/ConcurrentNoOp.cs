...
internal class ConcurrentNoOp : ConcurrencyCommandBase
{
    private ConcurrentNoOp(int threadCount, Duration executionTime) : base(threadCount, executionTime)
    {
    }
    ...
    internal static void Execute(string[] args)
    {
        ...
        var command = new ConcurrentNoOp(threadCount, Duration.FromSeconds(executionTimeSeconds));
        command.Execute();
    }
}
